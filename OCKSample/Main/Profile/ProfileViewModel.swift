...
extension ProfileViewModel {
    ...
    func saveContact() async throws {
        ...
        if var contactToUpdate = contact {
            ...
            let tempEmail = OCKLabeledValue(label: "email", value: email)
            let tempMessagingNumber = OCKLabeledValue(label: "messaging number", value: messagingNumber)
            let tempPhoneNumber = OCKLabeledValue(label: "phone number", value: phoneNumber)
            let tempOtherContactInfo = OCKLabeledValue(label: "other contact info", value: otherContactInfo)

            if contact?.emailAddresses != [tempEmail] {
                contactHasBeenUpdated = true
                contactToUpdate.emailAddresses = [tempEmail]
            }

            if contact?.messagingNumbers != [tempMessagingNumber] {
                contactHasBeenUpdated = true
                contactToUpdate.messagingNumbers = [tempMessagingNumber]
            }

            if contact?.phoneNumbers != [tempPhoneNumber] {
                contactHasBeenUpdated = true
                contactToUpdate.phoneNumbers = [tempPhoneNumber]
            }

            if contact?.otherContactInfo != [tempOtherContactInfo] {
                contactHasBeenUpdated = true
                contactToUpdate.otherContactInfo = [tempOtherContactInfo]
            }
